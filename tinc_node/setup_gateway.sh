...
if test -d /etc/tinc/machikado_network; then
  printf 'エラー: /etc/tinc/machikado_network は既に存在します\n' >&2
  printf 'セットアップを中止します\n' >&2
  printf 'セットアップをやりなおす場合は sudo rm -rf /etc/tinc/machikado_network してから再度セットアップスクリプトを実行してください\n' >&2
  exit 1
fi
...
# 設定用ディレクトリの作成
mkdir /etc/tinc/machikado_network
mkdir /etc/tinc/machikado_network/hosts

# 自ノードのノード定義の作成
# いまのところ特に設定する内容は無い
sed 's/{NODE_NAME}/'"${node_name}"'/' > /etc/tinc/machikado_network/hosts/"${node_name}" <<'EOF'
# {NODE_NAME}
EOF
...
sed 's/{IP_ADDRESS}/'"${ip_address}"'/' > /etc/tinc/machikado_network/tinc-up <<'EOF'
#!/bin/sh
...
cat > /etc/tinc/machikado_network/nat.iptables <<'EOF'
...
cat > /etc/tinc/machikado_network/tinc-down <<'EOF'
#!/bin/sh
...
sudo tincd -K -n machikado_network | cat
